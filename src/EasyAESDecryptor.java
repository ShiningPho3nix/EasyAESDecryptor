...
public class EasyAESDecryptor {
			...
			for (int hex1 = 1; hex1 <= 256; ++hex1) {
				for (int j = 0; j < 16; ++j) {
					for (int hex2 = 1; hex2 <= 256; ++hex2) {
						...
						cipherToKeyMap.put(DatatypeConverter.printBase64Binary(cipher.doFinal(PLAIN.getBytes("UTF-8"))),
								key);
					}
				}
			}
	...
	public static void main(String[] args) {
		/*
		 * try { Cipher cipher = Cipher.getInstance("AES/ECB/NoPadding"); byte[]
		 * keyBytes1 =
		 * DatatypeConverter.parseHexBinary("000000F5000000000000630000000000"); byte[]
		 * keyBytes2 =
		 * DatatypeConverter.parseHexBinary("0000000077000000B000000000000000"); Key
		 * key1 = new SecretKeySpec(keyBytes1, "AES"); Key key2 = new
		 * SecretKeySpec(keyBytes2, "AES");
		 *
		 * cipher.init(Cipher.ENCRYPT_MODE, key1); byte[] first =
		 * cipher.doFinal(PLAIN.getBytes("UTF-8"));
		 *
		 * cipher.init(Cipher.ENCRYPT_MODE, key2); byte[] second =
		 * cipher.doFinal(first);
		 *
		 * System.out.println(DatatypeConverter.printHexBinary(second));
		 * System.out.println("Matches?: " +
		 * Arrays.equals(DatatypeConverter.parseHexBinary(CIPHER), second));
		 *
		 * } catch (Exception e) { System.err.println(e.getCause().getClass() + ": " +
